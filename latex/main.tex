...
O crescimento do ecossistema de Open Finance e a expansão das soluções financeiras digitais têm aumentado a demanda por profissionais capazes de desenvolver integrações com APIs bancárias, sistemas de pagamento e mecanismos de autenticação seguros.
A identificação dessa necessidade baseou-se no mapeamento do cenário atual de desenvolvimento de software, no qual os ecossistemas bancários proprietários exigem certificações onerosas, chaves de acesso restritas e infraestruturas robustas apenas para a validação inicial de conceitos computacionais. 
O impacto desse problema reflete-se diretamente no sufocamento da inovação tecnológica local, limitando o surgimento de novas soluções financeiras inclusivas devido à escassez de ferramentas de simulação gratuitas ou de baixo custo para o público técnico.

Embora diversos bancos e fintechs ofereçam ambientes \sandbox{} para desenvolvedores, esses ambientes são geralmente voltados para a integração com sistemas de produção, exigindo credenciais específicas, dados reais (ou dados de teste específicos) e conformidade regulatória. Isso cria barreiras significativas para estudantes, 
desenvolvedores iniciantes e pequenas startups que buscam aprender, experimentar e validar protótipos sem riscos financeiros ou legais.

Como consequência, muitos estudantes restringem seu aprendizado aos conceitos teóricos ou utilizam APIs simuladas de propósito geral, que não reproduzem o comportamento de um sistema financeiro real.
Essa lacuna de conhecimento prático limita as oportunidades de aprendizado e aumenta a barreira de entrada para profissionais interessados em atuar no desenvolvimento de aplicações para o ecossistema financeiro digital.
% Escreva aqui o problema da comunidade.

\chapter{Solução Proposta}
Nossa solução propõe a criação de uma plataforma voltada à educação em tecnologias de integração financeira, com foco no Open Finance. A Jataí oferecerá um conjunto de produtos e serviços que permitirão aos estudantes, desenvolvedores iniciantes e pequenas startups acessar ambientes de teste simulados, consumir APIs financeiras fictícias, experimentar fluxos de autenticação e autorização, 
gerar dados financeiros sintéticos e validar protótipos sem depender de dados bancários reais ou de integrações com sistemas de produção.

A plataforma será desenvolvida e mantida por uma empresa júnior, permitindo que estudantes e desenvolvedores tenham acesso a um ambiente de experimentação semelhante ao encontrado em instituições financeiras, porém sem a necessidade de utilizar dados reais ou atender às exigências regulatórias presentes em ambientes de produção.

A plataforma inclui uma abordagem de hospedagem liver, onde o usuário poderá optar por utilizar a versão hospedada em nuvem na versão PRO ou executar a solução localmente (\textit{self-hosted})

% Escreva aqui a solução proposta.

\section{Benefícios esperados}
\begin{itemize}
  \item Ampliar o acesso ao aprendizado prático de integrações financeiras, reduzindo barreiras técnicas, financeiras e regulatórias para estudantes e desenvolvedores iniciantes.
  \item Estimular inovações tecnológicas no ecossistema financeiro a longo prazo, incentivando novas soluções inclusivas e criativas.
  \item Fortalecimento da formação acadêmica em tecnologia da informação.
  \item Promover a colaboração da comunidade tecnológica, incentivando a troca de conhecimento, experiências e boas práticas entre estudantes, desenvolvedores e profissionais do setor financeiro.
\end{itemize}
...
Simula integralmente o comportamento de endpoints bancários padronizados para consulta de saldo, extrato e histórico de transações, criação e consulta de cobranças, como boletos bancários simulados em contas de teste. Resolve a carência de dados estruturados para validações sintáticas e de interface. Pode ser utilizado em conjunto com o Jataí \textit{Pay Sandbox} para simular fluxos de pagamento e transferência de fundos. Seu intuito é fornecer recursos para o desenvolvimento de interfaces de usuário, testes de integração e prototipagem de aplicações financeiras.
...
Analisa tempo de resposta, latência e taxa de erro das requisições enviadas pelos sistemas dos clientes. Aponta gargalos no desempenho do código e auxilia na otimização de consultas. Podendo ser utilizado para testes de carga e simulação de instabilidades.
...
\centering
  ...
  \hline
  \textbf{Nível} & \textbf{Descrição} & \textbf{Preço estimado} & \textbf{Público-alvo} \\ \hline

  Gratuito &
  Acesso ao código fonte da aplicação para execução de todos os produtos Jataí localmente. &
  R\$ 0,00 & 
  Estudantes, pesquisadores e desenvolvedores iniciantes \\ \hline
  ...
  R\$ 19,90/mês & 
  Desenvolvedores autônomos, freelancers e pequenas equipes \\ \hline
